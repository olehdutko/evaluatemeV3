...
All backend REST endpoints use URL path versioning:
...
For list endpoints without pagination, `meta` may be omitted.
For single-resource endpoints, `data` contains the resource object.

## Pagination

List endpoints use cursor or offset pagination depending on use case.
...
- Breaking changes will be introduced under `/api/v2/` with at least one minor version deprecation period.
...
- Endpoint-specific contracts (auth, users, campaigns, etc.) are defined in their respective feature specs under `contracts/`.
- The frontend uses a typed REST client that validates responses against Zod schemas matching these conventions.
